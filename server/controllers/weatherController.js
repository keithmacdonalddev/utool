...
exports.getCurrentWeather = async (req, res, next) => {
    ...
    if (!primaryApiKey && !secondaryApiKey) {
        console.error('OpenWeatherMap API keys (Primary and Secondary) not configured.');
     ...
     if (!location) {
        return res.status(400).json({ success: false, message: 'Please provide a location query parameter (e.g., city name).' });
    }
    ...
    const attemptFetch = async (keyToTry) => {
        ...
        console.log(`Attempting weather fetch with key: ${keyToTry.substring(0, 5)}...`);
        const url = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(location)}&appid=${keyToTry}&units=metric`;
        try {
            ...
            if (response.data && response.data.main && response.data.weather) {
                 ...
                 console.warn('Received invalid data structure from OpenWeatherMap with key:', keyToTry.substring(0, 5));
                 return { success: false, error: new Error('Invalid data structure received'), statusCode: 500 };
            }
        } catch (err) {
            // Return the error object to be handled by the main logic
            console.error(`Weather API Error with key ${keyToTry.substring(0, 5)}...:`, err.response?.data || err.message);
    ...
    if (result && !result.success && result.statusCode === 401 && secondaryApiKey) {
    ...
    if (result && result.success) {
        ...
        } else if (finalError && finalError.message === 'Invalid data structure received') {
        ...
        if(finalError) console.error('Final Weather API Error:', finalError.response?.data || finalError.message);
