...
const KnowledgeBaseArticle = require('../models/KnowledgeBaseArticle'); // To check if article exists
const { ACCESS_LEVELS } = require('../config/permissions'); // For potential permission checks
...
exports.getCommentsForArticle = async (req, res, next) => {
    try {
        ...
        if (!articleExists) {
            return res.status(404).json({ success: false, message: 'Knowledge base article not found' });
        }
    ...
    try {
        ...
        if (parentCommentId) {
            ...
            if (!parentComment || parentComment.article.toString() !== articleId) {
                return res.status(400).json({ success: false, message: 'Invalid parent comment ID for this article' });
            }
        }
