...
const CommentForm = ({ articleId, parentCommentId = null, existingComment = null, onSubmitSuccess, onCancel }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            console.log('Submitting comment/reply:', { articleId, parentCommentId, content });
            if (existingComment) {
                // Update existing comment
                await api.put(`/comments/${existingComment._id}`, { content });
            } else {
                // Create new comment/reply
                await api.post(`/kb/${articleId}/comments`, { content, parentCommentId });
            }
    ...
    return (
        <form onSubmit={handleSubmit} className={`mt-2 ${parentCommentId || existingComment ? 'ml-8 pl-4 border-l-2' : ''}`}>
            <textarea
                value={content}
                onChange={(e) => setContent(e.target.value)}
                placeholder={parentCommentId ? "Write a reply..." : (existingComment ? "Edit comment..." : "Add a comment...")}
                rows="2"
            ...
            <div className="flex justify-end gap-2 mt-2">
                {onCancel && (
                     <button type="button" onClick={onCancel} className="text-xs text-gray-600 hover:underline" disabled={isSubmitting}>Cancel</button>
                )}
                <button type="submit" className="bg-blue-500 text-white px-3 py-1 rounded text-xs hover:bg-blue-600 disabled:opacity-50" disabled={isSubmitting}>
                    {isSubmitting ? 'Submitting...' : (existingComment ? 'Update' : (parentCommentId ? 'Reply' : 'Comment'))}
                </button>
            </div>
        </form>
    );
...
const CommentItem = ({ comment, articleId, onCommentAction }) => {
    ...
    const handleDelete = async () => {
         if (window.confirm('Are you sure you want to delete this comment? This cannot be undone.')) { // Added emphasis
             try {
                ...
                <div className="flex-1">
                    ...
                    {!showEditForm && (
                        <div className="mt-2 flex items-center space-x-4 text-xs text-gray-500">
                            {/* Actions */}
                            <button className="hover:text-blue-600 flex items-center" title="Like (Not Implemented)"><ThumbsUp size={14} className="mr-1" /> Like</button>
                            <button onClick={() => setShowReplyForm(!showReplyForm)} className="hover:text-blue-600 flex items-center" title="Reply"><MessageSquare size={14} className="mr-1" /> Reply</button>
                            {/* Show Edit/Delete if user is owner OR admin */}
                            {canModify && (
                                <>
                                    <button onClick={() => setShowEditForm(true)} className="hover:text-green-600 flex items-center" title="Edit"><Edit3 size={14} className="mr-1" /> Edit</button>
                                    <button onClick={handleDelete} className="hover:text-red-600 flex items-center" title="Delete"><Trash2 size={14} className="mr-1" /> Delete</button>
                                </>
                            )}
                        </div>
                    )}
