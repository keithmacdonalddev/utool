  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 mb-6 pt-4 border-t">
      ...
      <div className="flex justify-end mt-2">
        <button
          type="submit"
          className="bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700 disabled:opacity-50 shadow"
...
const CommentList = ({ articleId }) => {
  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      ...
      if (response.data?.success && Array.isArray(response.data.data)) {
        ...
        throw new Error(
          response.data?.message || 'Invalid data received for comments'
        );
      }
    } catch (err) {
  ...
  const renderComment = (comment) => (
    <div
      key={comment._id}
      className={comment.parentComment ? 'ml-8 pl-4 border-l-2' : ''}
    >
      <CommentItem
        comment={comment}
        articleId={articleId}
  ...
  return (
    <div className="mt-8 p-6 bg-card border border-dark-700 shadow-card rounded-lg">
      ...
      <NewCommentForm
        articleId={articleId}
        onSubmitSuccess={handleCommentAction}
      />
