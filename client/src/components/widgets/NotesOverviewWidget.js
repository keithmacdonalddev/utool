...
const NotesOverviewWidget = () => {
  ...
  return (
    <div className="p-4 border border-dark-700 rounded-xl shadow-card bg-card text-text">
      ...
      {isError && <p className="text-red-400 text-sm">{message || 'Error loading notes.'}</p>}
      {!isLoading && !isError && (
        <ul className="space-y-2">
          {recentNotes.length > 0 ? (
            recentNotes.map((note) => (
              <li key={note._id} className="border-b border-dark-700 pb-1 last:border-b-0">
                <p className="text-sm text-[#F8FAFC] truncate">{note.content}</p>
