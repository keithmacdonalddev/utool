...
const LoginPage = () => {
    ...
    useEffect(() => {
        ...
        if (isSuccess || user) {
            ...
            // Reset status after handling? Maybe not needed on redirect.
        }
    ...
    return (
        <div className="container mx-auto p-4 max-w-md"> {/* Added max-width */}
            <h1 className="text-3xl font-bold mb-6 text-center">Login</h1> {/* Centered and styled */}

            {/* Display Loading State */}
            {isLoading && <p className="text-center text-blue-500">Loading...</p>}

            {/* Display API Errors from Redux State */}
            {isError && message && <p className="text-center text-red-500 bg-red-100 p-2 rounded mb-4">{message}</p>}
            ...
            <form onSubmit={onSubmit} className="bg-card text-text shadow-card rounded-xl px-8 pt-6 pb-8 mb-4">
                <div className="mb-4">
                    ...
                    <input
                        className={`shadow appearance-none border border-dark-600 rounded w-full py-2 px-3 bg-dark-700 text-text leading-tight focus:outline-none focus:shadow-outline ${isError ? 'border-red-500' : ''} ${isLoading ? 'opacity-60' : ''}`}
                        id="email"
                        type="email"
                ...
                <div className="mb-6">
                    <label className="block text-[#F8FAFC] text-sm font-bold mb-2" htmlFor="password">
                        Password <span className="text-red-500">*</span>
                    </label>
                    <input
                        className={`shadow appearance-none border border-dark-600 rounded w-full py-2 px-3 bg-dark-700 text-text mb-3 leading-tight focus:outline-none focus:shadow-outline ${isError ? 'border-red-500' : ''} ${isLoading ? 'opacity-60' : ''}`}
                        id="password"
                        type="password"
                        placeholder="******************"
                        name="password"
                ...
                <p className="text-center text-[#C7C9D1] text-xs mt-4">
                    ...
                    <Link className="text-accent-purple font-bold hover:text-accent-blue hover:underline" to="/register">
                        Register here
                    </Link>
                </p>
