...
const TrashPage = () => {
  ...
  const { trash, isLoading, selectedNote } = useSelector((state) => state.notes);
  ...
  return (
    <div className="container mx-auto px-2 py-4">
      ...
      {showEditor && (
        <NoteEditorModal onClose={() => setShowEditor(false)} />
      )}
      {trash.length > 0 && (
        <div className="mt-4">
          ...
          <ul className="list-disc ml-6 text-sm text-gray-600">
            <li>
              <span className="font-medium">Restore:</span> Click "Edit" on a note, then "Restore" in the editor.
            </li>
            <li>
              <span className="font-medium">Permanent Delete:</span> Click "Edit" on a note, then "Delete" in the editor, or use the delete button in the list.
            </li>
          </ul>
        </div>
      )}
