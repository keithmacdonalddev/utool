...
import { Clipboard, Plus, X, Edit, Trash2 } from 'lucide-react';
 ...
 */
const SnippetsFeature = ({ activeCategory, setActiveCategory }) => {
  ...
  const handleCreateSubmit = (e) => {
    ...
    // Add active category tag if one is selected
    const snippetData = { ...newSnippet };
    if (activeCategory) {
      snippetData.tags = [...snippetData.tags, activeCategory.id];
    }
  ...
  const handleEditSubmit = (e) => {
    ...
    dispatch(updateSnippet(snippetToEdit))
      .unwrap()
      .then(() => {
  ...
  const filteredSnippets = snippets.filter((snippet) => {
    ...
    const matchesCategory = activeCategory
      ? snippet.tags?.includes(activeCategory.id)
      : true;
        ...
        <>
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-400">
              {activeCategory.count} snippet
              {activeCategory.count !== 1 ? 's' : ''}
            </p>
          </div>
